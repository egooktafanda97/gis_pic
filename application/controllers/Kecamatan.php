...
class Kecamatan extends CI_Controller
{
    ...
    public function detail($kode_kecamatan)
    {
        ...
        $dataId = $this->db->get_where('kecamatan', ['kode_kecamatan' => $kode_kecamatan])->row_array();
        $data = array(
            'title' => "Detail Data",
            'page' => $this->page . "detail",
            'script' => $this->page . "detailScript",
            'style' => $this->page . "style",
            'result' => $dataId,
            "kode" => $kode_kecamatan
        );
    ...
    public function getGeoJsonKecamatan($kode)
    {
        $geo = $this->db->get_where("geojson_kecamatan", ["kode_kecamatan" => $kode])->result_array();
        echo json_encode($this->BuidFormatGeoJson($geo));
    }
        ...
        foreach ($data as $value) {
            $geo = [
                ...
                "geometry" => [
                    ...
                    // "properties" => json_decode($value['propertis'], true)
                ]
            ];
            array_push($res, $geo);
        }
